...
<head>
	<meta charset="utf-8">
	<title><?php echo $banda_nombre ?> - Concert Manager</title>
...
<div class="ui middle aligned center aligned grid">
	...
	<div class="column">
		...
		<h3 class="top attached header">
			<?php echo $banda_nombre ?>
			<?php
				if ($owner){
			?>
			(<a href="<?php echo site_url("banda/administrar/{$banda_id}") ?>">Administrar</a>)
			<?php
				}
			?>
		</h3>
	  	...
	  	<div class="ui attached piled segment">
	  		...
	  		<table class="ui table">
	  			<tr>
	  				<td>Estado:</td>
	  				<td><?php echo $banda_estado == 1 ? 'Activa' : 'No activa' ?></td>
	  			</tr>
	  			<tr>
	  				<td>Géneros:</td>
	  				<td>
	  					<?php
	  						foreach ($generos as $i => $id) {

	  							echo "{$genero_model->getNombre($id)}".($i != count($generos)-1 ? ', ' : '');
	  			...
	  			<tr>
	  				<td>Pais:</td>	  				
	  			    <td><?php echo $banda_pais ?></td>  				
	  			</tr>
	  			<tr>
	  				<td>Región:</td>
	  				<td><?php echo $banda_region ?></td>
	  			</tr>
	  		</table>
